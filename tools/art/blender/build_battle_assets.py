...
Run with Blender 5.2:
  blender --background --python tools/art/blender/build_battle_assets.py
"""
...
OUTPUT = ROOT / "experiments/sudomer-diorama/assets/models/battle"
...
def flatten(source_name, output_name, excluded=()):
    ...
    return {"file": output.name, "source": f"tools/art/blender/source/{source_name}.blend", "pose_frame": 1, "animations": [], "vertices": len(vertices), "triangles": triangles, "source_objects": source_objects, "sha256": hashlib.sha256(output.read_bytes()).hexdigest()}


def create_unarmed_source():
    source_script = ROOT / "tools/art/blender/build_assets.py"
    namespace = {"__name__": "battle_asset_source", "__file__": str(source_script)}
    exec(compile(source_script.read_text(), namespace["__file__"], "exec"), namespace)
    namespace["reset"]()
