...
- `war_wagon` is rebuilt as a Hussite battle wagon (revised 23 September 2026). Tall horizontal planking with pointed stanchions, iron straps and plain horizontal shooting slots replaces the open farm-cart sides. A hinged lower board hangs on the outer (-Y) side between the wheels, closing the gap beneath the wagon as described for the wagon fort. Three shared-body crew (handgunner, crossbowman, flailman) fire over that side. The model stays inside the old wagon's plan outline and 3.94 m height, which set the renderer's formation envelope. The slots are deliberately not cross-shaped, to avoid reading as a crusader emblem. Earlier note: the original builder Only named tunic, skirt and sleeve parts plus existing banner cloth use `team_cloth`. Powder flasks, wood, metal and skin retain their original materials. The existing pale chalice remains linen because both Hussite and Prague wagon definitions already use that signal. The shared kit fixes the old polearm blade's collapsed bevel; its outline and the rest of the wagon remain unchanged.
...
| `war_wagon` | 5.285 × 2.940 × 3.930 | 3.195 | 10,872 |
