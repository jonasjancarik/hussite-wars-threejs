...
def main():
    ...
    for name in selected:
        ...
        bpy.ops.export_scene.gltf(filepath=str(MODELS/(name+'.glb')),export_format='GLB',use_selection=True,export_yup=True,export_animations=name=='cavalry',export_animation_mode='SCENE',export_nla_strips_merged_animation_name='HorseWalk',export_anim_scene_split_object=False,export_frame_range=True,export_force_sampling=True,export_materials='EXPORT',export_cameras=False,export_lights=False)
        manifest[name]={
            'file':name+'.glb','dimensions_blender_xyz_m':[round(v,3) for v in hi-lo],
