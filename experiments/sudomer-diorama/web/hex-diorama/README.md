...
The new renderer is under `views/3d/` (repository root). It adapts the TSL render graph, light rig, painted sky, photographic focus behavior and batching from `/Users/janca/projects/procedural-worlds` at commit `bada861a8d5c8cb7275a1b3d6e6a3f4ea4844cf4`, and uses that project's exported deciduous trees and shrubs. Exact provenance is in `views/3d/THIRD_PARTY_NOTICES.md` (repository root) and `assets/3d/models/vegetation/procedural-worlds/manifest.json` (repository root).
