...
import { COLS, HEX_RADIUS, ROWS, hexCenter } from "./hex-coordinates.ts";
import type { AuthoredTerrain } from "./terrain.ts";
import type { BattleSnapshot, HexCoord } from "./types.ts";

function key(coord: HexCoord): string { return `${coord.col},${coord.row}`; }

export function overlayGeometry(coord: HexCoord, terrain: Pick<AuthoredTerrain, "heightAt">, fill = false): THREE.BufferGeometry {
  const center = hexCenter(coord.col, coord.row);
  const vertices: number[] = [];
  const indices: number[] = [];
  const segments = 48;
  const bands = fill ? 8 : 1;
  for (let band = 0; band <= bands; band += 1) {
    const radius = fill ? HEX_RADIUS * 0.96 * band / bands : HEX_RADIUS * 0.96 - band * 0.19;
...
export class TacticalOverlays {
  ...
  public constructor(terrain: AuthoredTerrain) {
    ...
    for (let col = 0; col < COLS; col += 1) {
      for (let row = 0; row < ROWS; row += 1) {
        ...
        const ring = new THREE.Mesh(overlayGeometry({ col, row }, terrain), material);
        ring.renderOrder = 10;
        this.rings.set(`${col},${row}`, ring);
        this.group.add(ring);
        const fillMaterial = material.clone();
        const fill = new THREE.Mesh(overlayGeometry({ col, row }, terrain, true), fillMaterial);
        fill.renderOrder = 9;
        fill.visible = false;
  ...
  private refresh(): void {
    ...
    for (const [coordKey, ring] of this.rings) {
      ...
      let fillOpacity = 0;
      let color = 0xd9d1b5;
      if (moves.has(coordKey) || march.has(coordKey)) { opacity = 0.78; fillOpacity = 0.09; color = 0x92d4b1; }
      if (attacks.has(coordKey)) { opacity = 0.90; fillOpacity = 0.15; color = 0xef846a; }
      if (selected && coordKey === `${selected.col},${selected.row}`) { opacity = 1; fillOpacity = 0.20; color = 0xf5d47c; }
      if (this.hovered && coordKey === key(this.hovered)) { opacity = 1; fillOpacity = Math.max(fillOpacity, 0.24);
