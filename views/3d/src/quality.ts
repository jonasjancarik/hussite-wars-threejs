 ...
 * measurements (the WebGL fallback), intervals well under the target stand in.
 *
...
export class AutoQualityGovernor {
  ...
  public record(frameMs: number, now: number): QualityTier | null {
    ...
    if (median > this.targetMs * AUTO_QUALITY_SLOW_FACTOR && lower) {
      ...
      next = lower;
    } else if (higher && now >= this.retryAt[higher] && this.fits(higher, work, median)) {
      next = higher;
    }
    if (!next) return null;
    this.tier = next;
  ...
  private fits(higher: QualityTier, work: number | null, median: number): boolean {
    if (work === null) return median < this.targetMs * AUTO_QUALITY_HEADROOM;
