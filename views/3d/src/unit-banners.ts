...
export class UnitBanners {
  ...
  public update(snapshot: BattleSnapshot): void {
    ...
    for (const unit of this.units) {
      ...
      marker.button.dataset.selected = String(unit.id === snapshot.selectedUnitId);
      marker.button.setAttribute("aria-pressed", String(unit.id === snapshot.selectedUnitId));
      ...
      marker.action.textContent = unit.faction === snapshot.faction
        ? (presentation.actionAvailable ? "•" : "✓") : "";
      const summary = [unit.name, `${unit.health} / ${unit.maxHealth} HP`, presentation.moraleText,
      ...
      marker.button.disabled = snapshot.busy || snapshot.paused || snapshot.aiRunning;
  ...
  private createMarker(id: number): MarkerElements {
    ...
    button.append(flag, bar, action, badges, leader);
    ...
    const marker = { button, path, health, badges, action, leader };
