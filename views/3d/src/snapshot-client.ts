...
/** Standalone rules boot asynchronously while fetching their locale. */
export function waitForInitialSnapshot(readSnapshot: () => string, events: EventTarget): Promise<BattleSnapshot> {
  return new Promise((resolve, reject) => {
    const onSnapshot = (event: Event): void => resolve((event as CustomEvent<BattleSnapshot>).detail);
    events.addEventListener("sudomer-snapshot", onSnapshot, { once: true });
    try {
      const queued = readSnapshot();
      if (queued) {
        events.removeEventListener("sudomer-snapshot", onSnapshot);
        resolve(JSON.parse(queued) as BattleSnapshot);
      }
    } catch (error) {
      events.removeEventListener("sudomer-snapshot", onSnapshot);
      reject(error);
